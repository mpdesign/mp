...
class Db2 extends Mp {
	...
	function db($conn = "default"){
		...
		if(isset($config_db['mysqli'])){
			$this->db = dbiLink::getInstance()->db_config($conn)->connect();
		}else{
...
class dbLink {
	...
	function connect(  ) {
		...
		if ( !is_resource($this->{$this->db_conn}) ){
			...
			if ($this->pconnect) {
				$this->{$this->db_conn} = mysql_pconnect($this->host, $this->user, $this->password);
			}else {
				$this->{$this->db_conn} = mysql_connect($this->host, $this->user, $this->password, true);
			}
			if(is_resource($this->{$this->db_conn})) {
				if($this->dbname)mysql_select_db($this->dbname, $this->{$this->db_conn});
				mysql_query('set names utf8', $this->{$this->db_conn});
			} else {
				echo('Database upgrade, understanding!');
			}
	...
	function query($sql) {
		...
		$result = mysql_query($sql, $this->{$this->db_conn});
		$mysql_error = mysql_errno($this->{$this->db_conn});
		if ($this->abnormal && $mysql_error)  echo "MySQL error ".$mysql_error.": ".$mysql_error." <br>When executing:<br> $sql <br>";
...
class dbiLink {
	...
	function connect(  ) {
		...
		if ( !is_resource($this->{$this->db_conn}) ){
			$hp = explode(":", $this->host);
			$this->host = $hp[0];
			$this->port = $hp[1];
			$this->{$this->db_conn} = mysqli_connect($this->host, $this->user, $this->password, $this->dbname, $this->port);
