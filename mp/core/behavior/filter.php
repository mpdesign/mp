...
class Filter extends Mp {
	
	var $enable_xss = true;

	function _clean_data($str, $type = 'input', $unclean, $_standardize_newlines = true) {
		if (is_array($str)) {
			$new_array = array();
			foreach ($str as $key => $val) {
				$new_array[$this->_clean_keys($key)] = $this->_clean_data($val, $type, $unclean, $_standardize_newlines);
			}
			return $new_array;
		}
		...
		*/
		...
		if ($this->enable_xss) {
			Security::getInstance()->unclean = $unclean;
	 ...
	 * @return mixed
	 */
	public function input($input, $unclean=array()){
		return $this->_clean_data($input,'input',$unclean);
	}
	
	public function output($output){
		return Security::getInstance()->xss_clean($output,'output');
	}
 ...
 * @version   v1.0
 * @package   MP
 * @filesource http://codeigniter.com/user_guide/libraries/security.html
 * @date	Append 2013-02-19
 +------------------------------------------------------------------------------
 */
...
class Security {
	 ...
	 * @access protected
	 */
	protected $_csrf_token_name		= 'mp_csrf_token';
	/**
	 ...
	 * @access protected
	 */
	protected $_csrf_cookie_name	= 'mp_csrf_token';
	/**
	...
	public function __construct()
	{
		...
		$this->cookie_domain = config_item('cookie', 'domain');
		// Set the CSRF hash
		$this->_csrf_set_hash();
	...
	public function xss_clean($str, $is_image = FALSE)
	{
		...
		{
			$original = $str;
			if (!in_array('a', $this->unclean) && preg_match("/<a/i", $str))
			{
				$str = preg_replace_callback("#<a\s+([^>]*?)(>|$)#si", array($this, '_js_link_removal'), $str);
			}

			if (!in_array('img',$this->unclean) && preg_match("/<img/i", $str))
			{
